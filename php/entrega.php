...
<html lang="en" dir="ltr">
  <head>
    <meta charset="utf-8">
    <title>Entrega a jefe de area</title>
    <link rel="stylesheet" href="../css/enviodearchivos2.css">
  ...
  <body>
    ...
    <!--FIN DE LOS CIRCULOS-->
    <div class="container">
      <h2>Enviar y entregar trabajo final a jefe de area</h2>
      <form action="../subir2.php" method="post" enctype="multipart/form-data">
                <input type="hidden" value="<?php echo $email;?> " name="email">
                <input type="text" class="tarea" id="tarea" name="titulo" placeholder="INGRESA UN TITULO EN LA NOTA" required>
                <br>
                <label for="">Enviar al jefe</label>
                <select name="destinatario" id="destinatario" required>
                    <?php
                        $resultado = mysqli_query($conexion, "SELECT * FROM usuarios WHERE seccion='$session' and nivel='nivel1'");
                ...
                <br>
                <label for="">Proyecto relacionado a </label>
                <select name="proyecto" id="proyecto" required>
                    <?php
                    $resultado2 = mysqli_query($conexion, "SELECT * FROM sistema");
                        while($row = mysqli_fetch_array($resultado2)){
                            $proyect = $row[1];
                            echo"<option value='".$proyect."'>$proyect</option>";
                        }
                    ?>
                </select>
               <br>
               <label for="">Descripcion </label>
                ...
                <br>
                <input class="envio" type="submit" name="submit" value="Enviar archivo">
              <br>
            </form>
    </div>
